...
public class Barang extends javax.swing.JFrame {
    ...
    static final String DB_URL = "jdbc:mysql://localhost:3306/barang";
    ...
    private void tabelDataMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tabelDataMouseClicked
        ...
        if (baris != -1) {
            String kode = tabelData.getValueAt(baris, 1).toString();
            item_id = ambilId(kode);
            getData();
         }
    ...
    private void tblUpdateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_tblUpdateActionPerformed
        ...
        if (item_id != -1) {
            ...
            int stok = Integer.parseInt(txtStok.getText());
            int stok_min = Integer.parseInt(txtStokMin.getText());
            UpdateDenganId(Integer.toString(item_id), kode, nama, satuan, stok, stok_min);
            showtable();
            clear();
        } else {
    ...
    public void UpdateDenganId(String id, String kode, String nama, String satuan, int stok, int stokmin){
        try {
            Class.forName(JDBC_DRIVER);
            ...
            String sql = "UPDATE barang SET kd_brg=?, nm_brg=?, satuan=?, stok=?, stokmin=? WHERE id='"+id+"'";
    ...
    public boolean cekKodeBarang(String kode) {
        boolean kodeExists = false;
        try {
            Class.forName(JDBC_DRIVER);
            conn = DriverManager.getConnection(DB_URL, USER, PASS);
            stmt = conn.createStatement();
            String sql = "SELECT * FROM barang WHERE kd_brg='" + kode + "'";
            rs = stmt.executeQuery(sql);
            kodeExists = rs.next(); // Jika rs.next() true, berarti kode barang sudah ada
            rs.close();
            conn.close();
            stmt.close();
        } catch (Exception e) {
        ...
        public int ambilId(String kode) {
            int id = 0;
            try {
                Class.forName(JDBC_DRIVER);
                conn = DriverManager.getConnection(DB_URL, USER, PASS);
                stmt = conn.createStatement();
                String sql = "SELECT id FROM barang WHERE kd_brg=?";
                PreparedStatement pms = conn.prepareStatement(sql);
                pms.setString(1, kode);
                rs = pms.executeQuery();
                if (rs.next()) {
